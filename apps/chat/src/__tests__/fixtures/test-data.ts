 ...
 */
export function createTestThreadState(overrides?: Partial<ThreadState>): ThreadState {
    return {
        id: crypto.randomUUID(),
        messages: [],
        isLoading: false,
 ...
 */
export function createTestMessages(count: number, role: 'user' | 'assistant' | 'system' = 'user'): Message[] {
 ...
 */
export async function* createTestStream(chunks: string[]): AsyncIterable<string> {
