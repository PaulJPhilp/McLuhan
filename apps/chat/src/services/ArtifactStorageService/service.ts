...
export interface ArtifactStorageServiceSchema {
	...
	readonly getStorageStats: () => Effect.Effect<
		StorageStats,
		ArtifactStorageError
	>;
...
export class ArtifactStorageService extends Effect.Service<ArtifactStorageService>()(
	"ArtifactStorageService",
	{
		effect: Effect.fn(function* () {
			...
			const getErrorCode = (
				error: unknown,
			): "QUOTA_EXCEEDED" | "NOT_AVAILABLE" | "INVALID_DATA" | "UNKNOWN" => {
					...
					try {
						...
						try {
							const metadata = JSON.parse(
								localStorage.getItem(METADATA_KEY) || "{}",
							);
			...
			const getStorageStats = (): Effect.Effect<
				StorageStats,
				ArtifactStorageError
			> =>
				Effect.sync(() => {
