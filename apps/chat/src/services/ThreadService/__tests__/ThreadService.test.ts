...
describe("ThreadService", () => {
	...
	beforeEach(async () => {
		...
		await Effect.runPromise(
			Effect.provide(resetProgram, ThreadService.Default()),
		);
	});
	...
	describe("Service Initialization", () => {
		...
		it("should be accessible via Effect.Service", async () => {
			...
			const result = await Effect.runPromise(
				Effect.provide(program, ThreadService.Default()),
			);
	...
	describe("State Persistence", () => {
		it("should maintain state across multiple service calls", async () => {
			...
			await Effect.runPromise(
				Effect.provide(program1, ThreadService.Default()),
			);
			...
			const state = await Effect.runPromise(
				Effect.provide(program2, ThreadService.Default()),
			);

			// With the singleton pattern using shared Ref, state IS persisted across
			// separate Effect.runPromise calls, so we can verify the message was added
