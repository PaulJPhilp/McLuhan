...
import { createThreadActorConfig, ThreadState, Message, ThreadMessage } from '../ThreadActor'
...
describe('ThreadActor', () => {
    ...
    describe('ADD_MESSAGE', () => {
        it('should add a user message to empty state', async () => {
            ...
            const newState = await Effect.runPromise(config.receive(initialState, message))
        ...
        it('should append messages to existing state', async () => {
            ...
            const newState = await Effect.runPromise(config.receive(stateWithMessage, message))
        ...
        it('should generate unique IDs for messages', async () => {
            ...
            const state1 = await Effect.runPromise(config.receive(initialState, message))
            const state2 = await Effect.runPromise(config.receive(initialState, message))
    ...
    describe('CLEAR_MESSAGES', () => {
        it('should clear all messages from state', async () => {
            ...
            const newState = await Effect.runPromise(config.receive(stateWithMessages, message))

            expect(newState.messages).toHaveLength(0)
            expect(newState.error).toBeUndefined()
            expect(newState.lastUpdated).toBeGreaterThanOrEqual(stateWithMessages.lastUpdated)
        })
    ...
    describe('SET_LOADING', () => {
        ...
        it('should set loading to false', async () => {
            ...
            const newState = await Effect.runPromise(config.receive(stateWithLoading, message))
    ...
    describe('SET_ERROR', () => {
        it('should set error message', async () => {
            const message: ThreadMessage = { type: 'SET_ERROR', payload: 'Something went wrong' }
        ...
        it('should clear error when set to null', async () => {
            ...
            const newState = await Effect.runPromise(config.receive(stateWithError, message))
    ...
    describe('RETRY_LAST_MESSAGE', () => {
        ...
        it('should do nothing if no user messages exist', async () => {
            ...
            const newState = await Effect.runPromise(config.receive(stateWithOnlyAssistant, message))
        ...
        it('should keep only the last user message if it is the only message', async () => {
            const stateWithOneUserMessage = createTestThreadState({
                messages: [createTestMessage({ role: 'user', content: 'Only question' })],
            })

            const message: ThreadMessage = { type: 'RETRY_LAST_MESSAGE' }

            const newState = await Effect.runPromise(config.receive(stateWithOneUserMessage, message))
    ...
    describe('State Immutability', () => {
        it('should not mutate original state', async () => {
            const originalMessages = [createTestMessage({ role: 'user', content: 'Original' })]
