...
describe("ChatContext", () => {
	...
	describe("clearMessages", () => {
		it("should clear all messages", async () => {
			...
			await act(async () => {
				...
				// Wait a bit for atom refresh to complete
	...
	describe("retryLastMessage", () => {
		it("should call sendToThreadService with RETRY_LAST_MESSAGE", async () => {
			...
			const TestComponent = () => {
				...
				return (
					<div>
						<button
							onClick={() => void addMessage("user", "Question")}
			...
			await act(async () => {
				await screen.getByTestId("retry-button").click();
			});
		...
		it("should remove messages after last user message", async () => {
			// Test retry functionality: retryLastMessage removes messages after last user message
			const TestComponent = () => {
				const { addMessage, retryLastMessage, messages } = useChatContext();
			...
			await act(async () => {
				await screen.getByTestId("add-user-1").click();
			});
			await waitFor(
				() => {
					expect(screen.getByTestId("messages-count")).toHaveTextContent("1");
			...
			await act(async () => {
				await screen.getByTestId("add-assistant-1").click();
			});
			await waitFor(
				() => {
			...
			await act(async () => {
				await screen.getByTestId("add-user-2").click();
			});
			await waitFor(
				() => {
			...
			// Retry keeps all messages up to and including the last user message
			// So with [Question 1, Answer 1, Question 2], it keeps all 3 messages
			await act(async () => {
				await screen.getByTestId("retry-button").click();
			});
			await waitFor(
				() => {
					// RETRY_LAST_MESSAGE keeps messages up to and including last user message
					// So it keeps: Question 1, Answer 1, Question 2 (all 3)
					expect(screen.getByTestId("messages-count")).toHaveTextContent("3");
				},
				{ timeout: 5000, interval: 50 },
			);
		...
		it("should handle errors in sendMessage", async () => {
			...
			await waitFor(
				() => {
					...
					expect(
						errorText !== "none" ||
							screen.getByTestId("loading").textContent === "false",
					).toBeTruthy();
				},
				{ timeout: 5000 },
			);
